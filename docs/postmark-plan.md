...
- `forward.to` is documentation only — the real target is the Silo production delivery address configured in Settings → Silo (a full email on `mail.silo.day`, stored in the Keychain). The app never ships an address in the repo or rules file.
